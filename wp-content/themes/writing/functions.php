...
$social_networks = array("facebook" => "Facebook", "twitter" => "Twitter", "google-plus" =>  "Google Plus", "behance" => "Behance", "dribbble" => "Dribbble", "linkedin" => "Linked In", "youtube" => "Youtube", 'vimeo-square' => 'Vimeo', "vk" => "VK", "vine" => "Vine", "digg" => "Digg", "skype" => "Skype", "instagram" => "Instagram", "pinterest" => "Pinterest", "github" => "Github", "bitbucket" => "Bitbucket", "stack-overflow" => "Stack Overflow", "renren" => "Ren Ren", "flickr" => "Flickr", "soundcloud" => "Soundcloud", "steam" => "Steam", "qq" => "QQ", "slideshare" => "Slideshare", 'discord' => 'Discord', 'quora' => 'Quora', 'product-hunt' => 'Product Hunt', 'spotify' => 'Spotify', 'telegram' => 'Telegram', "rss" =>  "RSS");
...
function writing_setup() {
 ...
 require "inc/mods.php";
	...
	if (asalah_option('asalah_single_thumb_crop') != 'no') {
		set_post_thumbnail_size( $single_width, $single_height, true );
	} else {
		set_post_thumbnail_size( $single_width, $single_height, false );
	}
	...
	add_image_size('uncrop_masonry_blog', 455, 310, false);
	add_image_size('list_blog', 267, 205, true);
	add_image_size('uncrop_list_blog', 267, 205, false);
...
function load_custom_fonts($init) {
    ...
    if(empty($init['content_css'])) {  // Note #2
		...
		if ( get_theme_mod( 'asalah_blog_font_type') && (get_theme_mod( 'asalah_blog_font_type') != 'false')) {
			...
			$blog_font_type = str_replace('"', "'", $blog_font_type);
			$output .= 'font-family:'.$blog_font_type.' !important;';
		}
		if ((asalah_option('asalah_blog_font_size')) && (asalah_option('asalah_blog_font_size') != 'false')) {
			...
			$output .= "font-size:". $blog_font_size ."px !important;";
		}
...
/* Additional typography style for post editor */

add_action("admin_footer", "asalah_additional_editor_style");
function asalah_additional_editor_style() {
	global $post;
	$output = '';
	if ( get_theme_mod( 'asalah_blog_font_type') && (get_theme_mod( 'asalah_blog_font_type') != 'false')) {
		$blog_font_type = customizer_library_get_font_stack(get_theme_mod('asalah_blog_font_type'));
		$output = "<style>body {font-family:".$blog_font_type."}</style>";
	}
	echo $output;
}
/* --------
...
function asalah_scripts() {
	...
	if ( !is_rtl() ) {
		wp_enqueue_style( 'asalah-style', get_stylesheet_uri(), array(), '3.500');
	}else{
		wp_enqueue_style( 'asalah-rtl-base', get_template_directory_uri() . '/rtl_base.css', array(), '3.500' );
	}
	...
	wp_enqueue_script( 'asalah-script', get_template_directory_uri() . '/js/asalah.js', array( 'jquery' ), '3.500', true );
	if (asalah_option('asalah_lazyload_iframe_banner') == true) {
		wp_enqueue_script( 'asalah-lazyload', get_template_directory_uri() . '/js/lazysizes.min.js', array( 'jquery' ), '3.500', true );
	}
	...
	if (asalah_cross_option('asalah_pagination_style') == 'ajax') {
		global $wp_query;
		wp_enqueue_script( 'asalah-ajax-script', get_template_directory_uri() . '/js/ajaxpagination.js', array( 'jquery'), '3.500', true );
...
function asalah_customizer_live_preview() {
    wp_enqueue_script( 'asalah-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'jquery', 'customize-preview' ), '1.10', true );
}
...
function asalah_post_meta() {
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		...
		} elseif (asalah_cross_option('asalah_media_template_layout') == 'hide') {
...
// Converting Favicon to site icon option
...
function custom_wp_trim_excerpt($text, $limit, $more) {
$raw_excerpt = $text;
if ( '' == $text ) {
...
$more = '<a href="'.esc_url( get_permalink( get_the_ID() ) ).'" class="more_link more_link_dots">'.$excerpt_text.'</a>';
	...
	return custom_wp_trim_excerpt( '', $limit, $more );
    ...
    if ( get_theme_mod('asalah_enable_facebook_comments')):
        if ( get_theme_mod('asalah_enable_facebook_comments') == true):
            $facebook_app = asalah_get_fb_id();
						if (!isset($facebook_app) || $facebook_app === false) { return false;}
...
function asalah_comment($comment, $args, $depth) {
    ...
    switch ($comment->comment_type) :
        ...
        default :
                ?>
								<?php $comment_class = "media the_comment"; ?>
...
function asalah_pagination($id = '', $totalpages = '') {
    ...
    if (asalah_cross_option('asalah_pagination_style', $id) == 'num') {
		...
		echo '<div class="navigation_links navigation_prev ajax_nav_container ajax_load_more" data-cycle="'.$paged.'" data-posttype="post" data-postsperpage="'.$posts_per_page.'" data-loopfile="content.php" data-totalpages="'.$max.'" data-loaderimage="'.get_template_directory_uri().'/images/loader.gif" '.$lang_data.'" data-pageid="'.$id.'">';
		next_posts_link(__("Load More", "asalah"));
		echo '</div><div class="hidden ajax_data_helper" data-newtotalpages="'.$max.'" ></div>';
		}
	}
	} else{
    	if ( get_next_posts_link() ):
...
if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'carousel' )) {
		...
		/* Start Logo Style */
		 ...
		 if ( true == asalah_cross_option('asalah_center_logo')) {
				 ...
				 $output .= ".logo_wrapper {";
					 $output .= "text-align: center;";
					 if ( !(is_active_sidebar( 'sidebar-2' )  && asalah_cross_option('asalah_enable_sliding_sidebar') != 'no') && !((intval(asalah_cross_option('asalah_site_width')) < 701) && (intval(asalah_cross_option('asalah_site_width')) > 499) && (asalah_option('asalah_sidebar_position') != 'none')) ) {
						 $output .= "max-width: 100%;";
						 $output .= "width: 100%;";
					 } else {
						 $output .= "width: 80%;";
  					 $output .= "padding-left: 20%;";
					 }
				 $output .= "}";
				 if ( (is_active_sidebar( 'sidebar-2' )  && asalah_cross_option('asalah_enable_sliding_sidebar') != 'no') || ((intval(asalah_cross_option('asalah_site_width')) < 701) && (intval(asalah_cross_option('asalah_site_width')) > 499) && (asalah_option('asalah_sidebar_position') != 'none')) ) {
				 $output .= ".rtl .logo_wrapper {";
					 $output .= "padding-right: 20%;";
					 $output .= "padding-left: 0;";
				 $output .= "}";
				 }
		 ...
		 if (get_theme_mod('asalah_tagline_font_type') || ((asalah_option('asalah_tagline_font_size')) && (asalah_option('asalah_tagline_font_size') != 'false'))) {
			 $output .= ".logo_tagline.site_tagline {";
			...
			if ((asalah_option('asalah_tagline_font_size')) && (asalah_option('asalah_tagline_font_size') != 'false')) {
				$output .= 'font-size:	'.asalah_option('asalah_tagline_font_size').'px;';
			}

			if ((asalah_option('asalah_tagline_line_height')) && (asalah_option('asalah_tagline_line_height') != 'false')) {
				$line_height = asalah_option('asalah_tagline_line_height');

				$output .= "line-height:". $line_height ."px;";
			}
			$output .= "}";
		 }
...
if (!function_exists('asalah_banner_grid_post_count')) {
	function asalah_banner_grid_post_count()
	{
...
function my_update_notice() {
	...
	if (!isset($current_theme_version) || ($current_theme_version != $theme->get('Version'))) {
		?>
		<div class="updated is-dismissable notice sanabel-update" style="position:relative;">
			<h1>Writing</h1>
			<h2>You have updated to version <?php echo $theme->get("Version"); ?>, Look what we have brought:</h2>
				<b>What's new in the version:</b>
				<ul>
					<li>- Fix inline images in post content in mobile.</li>
					<li>- Fix share icons in the list with sidebar.</li>
					<li>- Improve Ajax load SEO friendly.</li>
...
add_filter( 'pt-ocdi/disable_pt_branding', '__return_true' );
 ...
 add_action( 'pt-ocdi/after_import', 'asalah_ocdi_after_import' );
	...
	function install_ocdi_plugin() {
			...
			add_action( 'admin_notices', 'install_ocdi_plugin' );

			add_action('admin_init', 'install_ocdi_plugin_notice');

			function install_ocdi_plugin_notice() {
